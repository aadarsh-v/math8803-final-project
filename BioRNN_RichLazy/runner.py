...
def run_edited_py(task='2AF', task_mode="ngym", num_runs=10, variable='orthogonal'):
    """Run edited.py multiple times with different sample comments."""
    for sample_num in range(num_runs):
        comment = f"sample_{sample_num}"
        cmd = [
            sys.executable,
        ...
        if result.returncode != 0:
            print(f"Error: new_edited.py exited with code {result.returncode}")
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--var_name', default='orthogonal', type=str, choices=['rr', 'spectral', 'orthogonal', 'sparse'],
                        help='Variable name for initialization')
    
    args = parser.parse_args()
    
    run_edited_py(task=args.task, task_mode=args.task_mode, num_runs=args.num_runs, variable=args.var_name)
